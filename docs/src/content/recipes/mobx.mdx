...
CodableJSON works seamlessly with MobX, allowing you to serialize reactive state while maintaining observability. The `project.spec.ts` test demonstrates a complete example.

## Basic MobX Integration

```typescript
import { observable, isObservableSet, isObservableMap, autorun, action, computed } from "mobx";
import { codableClass, codable, Coder, codableType, combineDecorators } from "codablejson";
...
```typescript
import { combineDecorators } from "codablejson";
...
// Observable codable (MobX + CodableJSON)
@$codable()
