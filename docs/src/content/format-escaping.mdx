...
CodableJSON use format like `{ $$Set: [1, 2, 3] }` to encode various types.

It is unlikely, but possible, that some data looking exactly like this will be passed to be encoded.

## Automatic Escaping

When your data contains keys that conflict with CodableJSON' internal format (like `$$Set`, `$$Map`, etc.), they are automatically escaped into object like `{ "~$$Set": [1, 2, 3] }`.
