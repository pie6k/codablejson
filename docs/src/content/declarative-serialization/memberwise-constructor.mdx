...
In order for CodableJSON to automatically handle your class, there is some convention you need to follow.

CodableJSON can automatically handle classes that:
- Have no constructor
...
// Still usable with CodableJSON, but will need to provide instance => [foo, bar] function
...
<Callout type="info">
Constructor is optional and not required for `@codableClass` to work. Under the hood, CodableJSON will automatically assign properties to the instance.
