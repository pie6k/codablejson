...
const MONACO_FONT_FAMILY = "Roboto Mono, 'SFMono-Regular', Consolas, 'Liberation Mono', Menlo, monospace";
    ...
    <Container>
      <Panel>
        <PanelHeader>
          ...
          {Object.entries(examples).map(([name, exampleCode]) => (
            <ExampleButton key={name} onClick={() => handleExampleClick(exampleCode)}>
              {name}
            </ExampleButton>
          ))}
        </PanelHeader>
      ...
      <Panel>
        <PanelHeader>
          ...
          {encodeTime !== null && (
            <TimingDisplay>
              ({encodeTime < 1 ? `${encodeTime.toFixed(2)}ms` : `${encodeTime.toFixed(2)}ms`})
            </TimingDisplay>
          )}
          {decodeBack && <ExampleButton onClick={handleDecodeBack}>Decode</ExampleButton>}
        </PanelHeader>
        <OutputContainer>
          {error && (
            <ErrorContainer>
